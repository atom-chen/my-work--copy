...
--read longlong
function ByteArray:readLongLong()
    local __, __v = string.unpack(self:readBuf(8), self:_getLC("o"))
    return __v
end
--
function ByteArray:writeLongLong(__longlong)
    local __s = string.pack("o", __longlong)
	self:writeBuf(__s)
	return self
end
...
--
function ByteArray:WriteStringBytesFixlen(__string, len)
	local __s = string.pack(self:_getLC("A"), __string)
	self:writeBuf(__s)
	local writeLen = string.len(__string)
	if len > writeLen then
		for i = 1, len - writeLen do
			self:writeByte(0);
		end
	end
	return self
...
--__string:utf8格式，需要转化为utf16
function ByteArray:WriteConvertStringFixlen(__string, len)
    len = len * 2
    --print(string.len(__string), __string)
    --local utf16String = UTF8ToUTF16(__string)
    local utf16String = LuaUtf8ToUtf16(__string)
    local __s = string.pack(self:_getLC("A"), utf16String)
    self:writeBuf(__s)
    local writeLen = string.len(utf16String)
    --dump(writeLen, "writeLen")
    --dump(utf16String, "utf16String")
    assert(writeLen <= len, "WriteConvertStringFixlen invlaid len")
    if len > writeLen then
        for i = 1, len - writeLen do
            self:writeByte(0);
        end
...
--   (-1 or nil ：读到包的结尾处)
function ByteArray:readConvertString(__len)
	if __len == 0 then return "" end
	self:_checkAvailable()
    if __len == nil or __len == -1 then
        __len = self:getAvailable()
    else
        __len = __len * 2
    end
	local utf16Str =  self:readBuf(__len)
	--return UTF16ToUTF8(utf16Str)
    return LuaUtf16ToUtf8(utf16Str)
end
...
--
function ByteArray:InsertDword(newValue, pos)
    local len = #self._buf
    for i = len , pos do
        self._buf[i + 5] = self._buf
    end
    self:setPos(pos)
    self:writeULong(newValue)
end
return ByteArray
